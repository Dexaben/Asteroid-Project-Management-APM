...
namespace Asteroid_Project_ManagerCEO
{
    public partial class proje_detay : Form
    {
        ...
        private void proje_detay_Load(object sender, EventArgs e)
        {
            ...
            table = F.SQL_SELECT_DATATABLE(komut, "Hata:Proje çalışan bilgileri alınamadı.", Color.Red, 4000);
            ...
            if (table != null)
            {
                ...
                {
                    dataTable_calisan.Rows.Add(Convert.ToInt32(table.Rows[i]["worker_id"]),table.Rows[i]["worker_name"].ToString(),F.CONVERT_BYTE_ARRAY_TO_IMAGE(table.Rows[i]["worker_image"]),Convert.ToInt32(table.Rows[i]["uzerindeki_gorev_say"]),Convert.ToInt32(table.Rows[i]["tamamladigi_gorev"]));
                }
            }
            ...
            komut.Parameters.AddWithValue("@PROJECT_ID", project_id);
            table = F.SQL_SELECT_DATATABLE(komut, "Hata:Proje bilgileri alınamadı.", Color.Red, 4000);
            if (table !=null)
            {
            ...
            for (int i = 0; i < table.Rows.Count; i++)
            {
                dataTable.Rows.Add(Convert.ToInt32(table.Rows[i]["task_id"]), table.Rows[i]["task_name"].ToString(), table.Rows[i]["task_status"].ToString(), F.Tarih_Converter_DAY_HOUR_MINUTE(table.Rows[i]["task_start_date"].ToString()), F.Tarih_Converter_DAY_HOUR_MINUTE(table.Rows[i]["task_finish_date"].ToString()), Convert.ToInt32(table.Rows[i]["calisan_sayisi"]));
            }
