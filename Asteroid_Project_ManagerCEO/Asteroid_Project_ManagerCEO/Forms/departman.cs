...
namespace Asteroid_Project_ManagerCEO.Forms
{
    public partial class departman : Form
    {
        ...
        {
            F.FORM_AC(new departman_ekle(), true);
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (departmanlar_datagrid.Rows.Count > 0)
            {
                ...
                F.FORM_AC(departman_Guncelle, true);
            }
            else F.DURUM_LABEL("Hata: Departman verisi bulunamadı.", Color.Purple, 2000);
        }
        int DEPARTMENT_PROJECTS_COUNT(int department_id) //DEPARTMAN PROJELERİ
        {
            ...
            table = F.SQL_SELECT_DATATABLE(komut, "Hata:Departman bilgisi alınamadı.", Color.Red, 3000);
            if (table != null)
            {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (departmanlar_datagrid.Rows.Count > 0)
            {
                ...
                if (dialog == DialogResult.Yes)
                {
                    ...
                    if (F.SQL_EXECUTENONQUERY(komut, "Hata:Departman bilgisi silinemedi.", Color.Red, 3000))
                    {
        ...
        private void row_selected(object sender, DataGridViewCellEventArgs e)
        {
            if (departmanlar_datagrid.Rows.Count > 0)
            {
                ...
                table = F.SQL_SELECT_DATATABLE(komut, "Hata:Departman verisi alınamadı.", Color.Red, 3000);
                if(table !=null)
                {
                    departman_resmi.Image = F.CONVERT_BYTE_ARRAY_TO_IMAGE(table.Rows[0]["department_logo"]);
            ...
            {
                F.DURUM_LABEL("Hata:Departman verisi bulunamadı.", Color.Purple, 2000);
        ...
        {
            F.OpenImage(departman_resmi.Image);
        }
    }
}
