...
namespace Asteroid_Project_ManagerCEO.Forms
{
    public partial class calisan : Form
    {
        ...
        private void calisan_Load(object sender, EventArgs e)
        {
            ...
            Scripts.SQL.SQL_COMMAND SqlCommand = new Scripts.SQL.SQL_COMMAND(komut, "Hata:Çalışan verileri alınamadı.", Color.Red, 4000);
            ...
            if (table !=null)
            {
                ...
                {
                    dataTable.Rows.Add(Convert.ToInt32(table.Rows[z]["worker_id"]), Scripts.Tools.ImageTools.CONVERT_BYTE_ARRAY_TO_IMAGE(table.Rows[z]["worker_image"]), table.Rows[z]["worker_name"].ToString(), Convert.ToBoolean(table.Rows[z]["worker_onay"]));
                }
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (calisan_datagrid.Rows.Count > 0)
            {
                i = calisan_datagrid.SelectedRows[0].Index;
                    ...
                    Scripts.Tools.LogTools.LOG_ENTER(0, $"{Environment.MachineName} bilgisayarından {calisan_datagrid.Rows[i].Cells[2].Value} çalışanının onay durumu güncellendi.(Onaysız => ONAYLI)",Scripts.SQL.SqlQueries.GET_SERVER_DATE());
                     }
                else
                    {
                    ...
                    Scripts.Tools.LogTools.LOG_ENTER(0, $"{Environment.MachineName} bilgisayarından {calisan_datagrid.Rows[i].Cells[2].Value} çalışanının onay durumu güncellendi.(Onaylı => ONAYSIZ)", Scripts.SQL.SqlQueries.GET_SERVER_DATE());
                }
                int a = Convert.ToInt32(calisan_datagrid.Rows[i].Cells[0].Value);
                    komut.Parameters.AddWithValue("@SECIM", a);
                Scripts.SQL.SQL_COMMAND SqlCommand = new Scripts.SQL.SQL_COMMAND(komut,"Hata:Çalışan bilgisi güncellenemedi",Color.Red,3000);
            ...
            else Scripts.Form.Status.STATUS_LABEL("Hata:Çalışan verisi bulunamadı.", Color.Purple, 2000);
        }
        private void button2_Click(object sender, EventArgs e)
        {
           Scripts.Form.FormManager.FORM_AC(new calisan_ekle(), true);
        }

        private void button3_Click(object sender, EventArgs e)
        {
            if (calisan_datagrid.Rows.Count > 0)
                Scripts.Form.FormManager.FORM_AC(new calisan_guncelle(Convert.ToInt32(calisan_datagrid.SelectedRows[0].Cells[0].Value)), true);
            else Scripts.Form.Status.STATUS_LABEL("Hata:Çalışan verisi bulunamadı.", Color.Purple, 2000);
        }
        int WORKER_MANAGED_PROJECT_COUNT(int worker_id)
        {
            ...
            Scripts.SQL.SQL_COMMAND SqlCommand = new Scripts.SQL.SQL_COMMAND(komut, "Hata:Çalışan bilgisi alınamadı.", Color.Red, 3000);
            table = Scripts.SQL.SqlQueries.SQL_SELECT_DATATABLE(SqlCommand);
            if (table != null)
            {
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            if (calisan_datagrid.Rows.Count > 0)
            {
                ...
                if(Scripts.SQL.SqlQueries.WORKER_ROLE_CALL_BY_ID(Convert.ToInt32(calisan_datagrid.SelectedRows[0].Cells[0].Value)).Length >0)
                {
                    message += " \nBu çalışanın " +Scripts.SQL.SqlQueries.WORKER_ROLE_CALL_BY_ID(Convert.ToInt32(calisan_datagrid.SelectedRows[0].Cells[0].Value)).Length + " adet pozisyon kaydı bulunuyor.";
                }
                if(WORKER_MANAGED_PROJECT_COUNT(Convert.ToInt32(calisan_datagrid.SelectedRows[0].Cells[0].Value))>0)
                {
                    message += " \nBu çalışanın " + WORKER_MANAGED_PROJECT_COUNT(Convert.ToInt32(calisan_datagrid.SelectedRows[0].Cells[0].Value)).ToString()+ " adet projeye yöneticilik yapmakta.";
                    if (PROJECT_WORKERS_COUNT(Convert.ToInt32(calisan_datagrid.SelectedRows[0].Cells[0].Value)) > 0)
                    {
                        message += " \nBu projelerde " + PROJECT_WORKERS_COUNT(Convert.ToInt32(calisan_datagrid.SelectedRows[0].Cells[0].Value)).ToString() + " adet çalışan görev alıyor.";
                    }
                    if(PROJECT_TASKS_COUNT(Convert.ToInt32(calisan_datagrid.SelectedRows[0].Cells[0].Value))>0)
                    {
                        message += " \nBu projelerde " + PROJECT_TASKS_COUNT(Convert.ToInt32(calisan_datagrid.SelectedRows[0].Cells[0].Value)).ToString() + " adet görev bulunuyor.";
                    }
                }
                DialogResult dialog = MessageBox.Show(message, "Çalışan Kaydı Siliniyor", MessageBoxButtons.YesNo);
                if (dialog == DialogResult.Yes)
                {
                    ...
                    Scripts.SQL.SQL_COMMAND SqlCommand = new Scripts.SQL.SQL_COMMAND(komut, "Hata:Çalışan verisi silinemedi.", Color.Red, 3000);
                    if (Scripts.SQL.SqlSetQueries.SQL_EXECUTENONQUERY(SqlCommand))
                    {
                        komut = new SqlCommand("DELETE FROM PROJECT_WORKERS WHERE worker_id=@id");
                        komut.Parameters.AddWithValue("@id", id);
                   SqlCommand = new Scripts.SQL.SQL_COMMAND(komut, "Hata:Çalışan verisi silinemedi.", Color.Red, 4000);
                        if (Scripts.SQL.SqlSetQueries.SQL_EXECUTENONQUERY(SqlCommand))
                        {
                            ...
                            if (Scripts.SQL.SqlSetQueries.SQL_EXECUTENONQUERY(SqlCommand))
                            {
                               ...
                               SqlCommand = new Scripts.SQL.SQL_COMMAND(komut, "Hata:Çalışan verisi silinemedi.", Color.Red, 4000);
                                if (Scripts.SQL.SqlSetQueries.SQL_EXECUTENONQUERY(SqlCommand))
                                {
                                    ...
                                    komut.Parameters.AddWithValue("@id", id);
                                    SqlCommand = new Scripts.SQL.SQL_COMMAND(komut, "Hata:Görev verileri silinemedi.", Color.Red, 4000);
                                    table = Scripts.SQL.SqlQueries.SQL_SELECT_DATATABLE(SqlCommand);
                                    if (table != null)
                                    {
                                        for (int z = 0;z< table.Rows.Count; z++)
                                        {
                                            ...
                                            komut.Parameters.AddWithValue("@PROJECT_ID", Convert.ToInt32(table.Rows[z]["project_id"]));
                                             SqlCommand = new Scripts.SQL.SQL_COMMAND(komut, "Hata:Görev çalışanları silinemedi.", Color.Red, 4000);
                                            if (Scripts.SQL.SqlSetQueries.SQL_EXECUTENONQUERY(SqlCommand))
                                            {
                                                ...
                                                komut.Parameters.AddWithValue("@PROJECT_ID", Convert.ToInt32(table.Rows[z]["project_id"]));
                                               SqlCommand = new Scripts.SQL.SQL_COMMAND(komut, "Hata:Görev resimleri silinemedi.", Color.Red, 4000);
                                                if (Scripts.SQL.SqlSetQueries.SQL_EXECUTENONQUERY(SqlCommand))
                                                {
                                                    komut = new SqlCommand("DELETE FROM TASKS WHERE project_id = @PROJECT_ID");
                                                    komut.Parameters.AddWithValue("@PROJECT_ID", Convert.ToInt32(table.Rows[z]["project_id"]));
                                                    SqlCommand = new Scripts.SQL.SQL_COMMAND(komut, "Hata:Görevler silinemedi.", Color.Red, 4000);
                                                    if (Scripts.SQL.SqlSetQueries.SQL_EXECUTENONQUERY(SqlCommand))
                                                    {
                                            ...
                                            Scripts.Tools.LogTools.LOG_ENTER(0, $"{Environment.MachineName} bilgisayarından (ID-{id}) {calisan_datagrid.SelectedRows[0].Cells[2].Value} adlı çalışan SİLİNDİ.Bu işlem ile çalışanın pozisyon kayıtları,Proje Yöneticiliği yaptığı projeler, bu projelerin görevleri ve içerisindeki veriler SİLİNDİ.", Scripts.SQL.SqlQueries.GET_SERVER_DATE());
                                            calisan_Load(this, null);
        ...
        private void row_selected(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (calisan_datagrid.Rows.Count > 0)
            {
                ...
                if (table != null)
                {
                    ...
                    pozisyon_listview.Items.Clear();
                        ...
                        pozisyon_listview.Items.AddRange(Scripts.SQL.SqlQueries.WORKER_ROLE_CALL_BY_ID(Convert.ToInt32(calisan_datagrid.SelectedRows[0].Cells[0].Value)));
            ...
            {
                Scripts.Form.Status.STATUS_LABEL("Hata:Çalışan verisi bulunamadı.", Color.Purple, 2000);
