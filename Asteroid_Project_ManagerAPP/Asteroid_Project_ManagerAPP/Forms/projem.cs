...
namespace Asteroid_Project_ManagerAPP
{
    public partial class projem : Form
    {
        ...
        {
            F.LOG_ENTER(a.worker_id, $"{F.WORKER_NAME_BY_WORKER_ID(a.worker_id)} adlı çalışan {F.PROJECT_NAME_BY_PROJECT_ID(a.project_id)} projesinden çıkış yaptı.Proje ID:{a.project_id}", F.GET_SERVER_DATE());
            a.project_id = 0;
        ...
        public void projem_Load(object sender, EventArgs e)
        {
            ...
            if (table.Rows.Count != 0)
            {
                ...
                proje_baslangic_tarihi.Text = F.Tarih_Converter_DAY_HOUR_MINUTE(table.Rows[0]["project_start_date"].ToString());
                proje_bitis_tarihi.Text = F.Tarih_Converter_DAY_HOUR_MINUTE(table.Rows[0]["project_finish_date"].ToString());
            ...
            komut.Parameters.AddWithValue("@PROJECT_ID", a.project_id);
            table = F.SQL_SELECT_DATATABLE(komut, "Hata:Aktif görevler alınırken sorun oluştu.", System.Drawing.Color.Red, 4000);
            ...
            if (table.Rows.Count != 0)
            {
                ...
                {
                    dataTable.Rows.Add(Convert.ToInt32(table.Rows[i]["task_id"]), table.Rows[i]["task_name"].ToString(), table.Rows[i]["task_urgency"].ToString(), table.Rows[i]["task_status"].ToString(), F.Tarih_Converter_DAY_HOUR_MINUTE(table.Rows[i]["task_finish_date"].ToString()), Convert.ToInt32(table.Rows[i]["gorevdeki_calisanlar"]));
                }
            }
            ...
            if (table.Rows.Count != 0)
            {
                for (int i = 0; i < table.Rows.Count; i++)
                {
                    dataTable.Rows.Add(Convert.ToInt32(table.Rows[i]["worker_id"]), F.CONVERT_BYTE_ARRAY_TO_IMAGE(table.Rows[i]["worker_image"]), table.Rows[i]["worker_name"].ToString(),Convert.ToInt32(table.Rows[i]["calisan_proje_say"]), Convert.ToInt32(table.Rows[i]["gorev_calisan_say"]));
                }
            }
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            F.FORM_AC(new Forms.proje_guncelle(), true);
        }
