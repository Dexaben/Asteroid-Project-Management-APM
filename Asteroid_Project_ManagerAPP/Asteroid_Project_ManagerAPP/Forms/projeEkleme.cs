    ...
    public partial class projeEkleme : Form
    {
        ...
        void kaydet()
        {
            ...
            if (F.SQL_EXECUTENONQUERY(komut,"Hata:Proje veritabanına eklenemedi.",Color.Red,3000))
            {
                ...
                {
                    workers += $" {F.WORKER_NAME_BY_WORKER_ID(Convert.ToInt32(projeye_eklenenler_datagrid.Rows[i].Cells[0].Value))} ,";
                    ...
                    komut.Parameters.AddWithValue("@PROJECT_NAME", proje_adi.Text);
                    if (!F.SQL_EXECUTENONQUERY(komut, "Hata:Proje çalışan bilgisi eklenemedi.", Color.Red, 3000))
                    { return; }
                }
                F.LOG_ENTER(a.worker_id, $"{F.WORKER_NAME_BY_WORKER_ID(a.worker_id)} adlı çalışan {proje_adi.Text} projesini oluşturdu.Proje Yöneticisi:{proje_yoneticileri.Text},Proje Başlangıç Tarihi:{time_Start},Proje Bitiş Tarihi:{time_Finish},Proje Departmanı:{departmanlar.Text},Proje Çalışanları:{workers}", F.GET_SERVER_DATE());
        ...
        private void projeEkleme_Load(object sender, EventArgs e)
        {
            ...
            proje_baslangic_tarihi.MinDate = F.GET_SERVER_DATE();
            proje_bitis_tarihi.MinDate = F.GET_SERVER_DATE().AddDays(1);
            ...
            if (table != null)
            {
                ...
                {
                    dataTable.Rows.Add(Convert.ToInt32(table.Rows[i]["worker_id"]), table.Rows[i]["worker_name"].ToString(), F.CONVERT_BYTE_ARRAY_TO_IMAGE(table.Rows[i]["worker_image"]), Convert.ToInt32(table.Rows[i]["ProjeSayisi"]), Convert.ToInt32(table.Rows[i]["TamamlanmisGorevSay"]));
                }
            }
            ...
            {
                F.DURUM_LABEL("Proje Ekleyebilmeniz için en az bir DEPARTMAN'a ihtiyacınız var", Color.PaleVioletRed, 4000);
        ...
        private void dataGridView1_DoubleClick(object sender, EventArgs e)
        {
            for (int i = 0; i < projeye_eklenenler_datagrid.Rows.Count; i++)
            {
                ...
                {
                    F.DURUM_LABEL("Bu çalışan zaten ekli.", Color.BlueViolet, 2000);
                    return;
