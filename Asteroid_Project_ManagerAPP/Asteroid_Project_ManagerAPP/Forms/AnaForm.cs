...
namespace Asteroid_Project_ManagerAPP
{
    public partial class AnaForm : Form
    {
        ...
        {
            timer_baslangic = DateTime.Now-F.GET_SERVER_DATE(); //LOCAL MAKİNE ZAMANI İLE SERVER MAKİNESİ ZAMANININ , ZAMAN FARKINI ALIYOR.
            sonGirisZamani = F.GET_SERVER_DATE(); //UYGULAMAYA GİRİŞ ZAMANI ALINIYOR.
            panel_tarih.Text = F.GET_SERVER_DATE().ToString("dddd, dd MMMM yyyy");
            timer1.Start();
            Controls.OfType<MdiClient>().FirstOrDefault().BackColor = Color.White; //MDI CONTAINER ARKA PLAN RENGİNİ BEYAZ YAPIYOR.
            menuStrip1.Visible = false;
            F.FORM_AC(new calisan_giris(), true);
            F.DURUM_LABEL("Durum : Kayıt,giriş bekleniyor.",Color.White);
        }
        public void ANAFORM_BILGILER_GUNCELLE()
        {
            ...
            if (worker_id != 0) //ANA PANEL UZERİNDEKİ PROFİL BİLGİLERİNİ SENKRONİZE EDER
            {
                ...
                table = F.SQL_SELECT_DATATABLE(komut, "Hata Bilgiler güncellenirken sorun oluştu.", Color.Red, 3000);
                if (table.Rows.Count != 0)
                {
                    profil_bilgileri_groupBox.Text = "Profil Bilgileri - ID(" + table.Rows[0]["worker_id"].ToString() + ")";
                    profil_resmi.Image = F.CONVERT_BYTE_ARRAY_TO_IMAGE(table.Rows[0]["worker_image"]);
                    textBox6.Text = (table.Rows[0]["worker_name"]).ToString();
                    if (F.WORKER_HAS_ROLE_BY_WORKER_ID_AND_WORKER_ROLE(Convert.ToInt32(table.Rows[0]["worker_id"]), "Proje Yöneticisi"))
                    {
            ...
            if (project_id != 0) //ANA PANEL UZERİNDEKİ GOREV VE PROJE BİLGİLERİNİ SENKRONİZE EDER
            {
                ...
                table = F.SQL_SELECT_DATATABLE(komut, "Hata:En yakın görev bilgisi alınırken sorun oluştu.", Color.Red, 2000);
                ...
                for (int i = 0; i < table.Rows.Count; i++)
                {
                    ...
                    if (t.task_finish_date >= F.GET_SERVER_DATE()) tasks.Add(t);
                }
                if (tasks.Count > 0)
                {
                    bubbleSort(tasks);
                    en_yakin_gorev.Text = tasks[0].task_name;
                    gorev_tarih.Text = F.Tarih_Converter_DAY_HOUR_MINUTE(tasks[0].task_finish_date.ToString());
                ...
                komut.Parameters.AddWithValue("@project_id", project_id);
                table = F.SQL_SELECT_DATATABLE(komut, "Hata:Proje bilgileri alınırken hata oluştu.",Color.Red, 2000);
                if(table.Rows.Count != 0)
                {
        ...
        private void sohbetToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (project_id != 0)
            {
               F.FORM_AC(new sohbet(), true);
            }
            else F.DURUM_LABEL("Durum: Herhangibir proje seçimi yapılmamış.", Color.White, 2000);
        }
        ...
        {
            F.OpenImage(profil_resmi.Image);
        }
        ...
        private void AnaForm_FormClosed(object sender, FormClosedEventArgs e) //PROGRAM KAPATILDIGINDA ÇALIŞACAK FONKSIYON
        {
            if(worker_id != 0)
            {
                F.LOG_ENTER(worker_id, F.WORKER_NAME_BY_WORKER_ID(worker_id)+" adlı çalışan uygulamadan çıkış yaptı. Uygulamada kalma süresi : "+ (F.GET_SERVER_DATE() - sonGirisZamani), F.GET_SERVER_DATE());
            }
        }
