...
namespace Asteroid_Project_ManagerAPP
{
    public partial class FUNCTIONS //SQL FUNCTIONS
    {
        ...
        public string WORKER_NAME_BY_WORKER_ID(int worker_id)
        {
            try
            {
                sqlBaglan = SQL_BAGLAN(sqlBaglan);
                sqlBaglan.Open();
                SqlCommand komut = new SqlCommand("SELECT worker_name FROM WORKERS WHERE worker_id=@WORKER_ID", sqlBaglan);
                komut.Parameters.AddWithValue("@WORKER_ID", worker_id);
                System.Data.DataTable table = new System.Data.DataTable();
                table = SQL_SELECT_DATATABLE(komut, "Hata:Çalışan bilgileri veritabanından alınamadı.", System.Drawing.Color.Red, 3000);
                if (table != null)
                ...
                else DURUM_LABEL("Hata: Veritabanında kayıtlı çalışan bulunamadı.", System.Drawing.Color.Yellow, 2000);
            }
        ...
        public string PROJECT_NAME_BY_PROJECT_ID(int project_id)
        {
            try
            {
                sqlBaglan = SQL_BAGLAN(sqlBaglan);
                sqlBaglan.Open();
                SqlCommand komut = new SqlCommand("SELECT project_name FROM PROJECTS WHERE project_id=@PROJECT_ID", sqlBaglan);
                komut.Parameters.AddWithValue("@PROJECT_ID", project_id);
                System.Data.DataTable table = new System.Data.DataTable();
                table = SQL_SELECT_DATATABLE(komut, "Hata:Proje bilgileri veritabanından alınamadı.", System.Drawing.Color.Red, 3000);
                if (table != null)
                {
                    return table.Rows[0]["project_name"].ToString();
                }
                else DURUM_LABEL("Hata: Veritabanında kayıtlı proje bulunamadı.", System.Drawing.Color.Yellow, 2000);
            }
            catch { DURUM_LABEL("Hata: Veritabanından bilgi alınırken hata oluştu.", System.Drawing.Color.Red, 2000); }
            finally
            {
        ...
        public bool PROJE_YONETICISI_KONTROL(int worker_id, int project_id)
        {
            try
            {
                sqlBaglan = SQL_BAGLAN(sqlBaglan);
                sqlBaglan.Open();
                SqlCommand komut = new SqlCommand("SELECT P.project_manager FROM PROJECTS P WHERE P.project_id=@PROJECT_ID", sqlBaglan);
                komut.Parameters.AddWithValue("@PROJECT_ID", project_id);
                SqlDataReader veri = komut.ExecuteReader();
                if (veri.Read())
                {
    ...
    public partial class FUNCTIONS
    {
        ...
        public System.Drawing.Color LIGHTER_COLOR(System.Drawing.Color color, float correctionfactory = 50f)
        {
        ...
        public System.Drawing.Color DARKER_COLOR(System.Drawing.Color color, float correctionfactory = 50f)
        {
